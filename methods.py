...
def get_ip():
    """
    Find the local IP to be scanned. Utilize netifaces to be platform
    independent.
    """
...
class Controller(object):
    ...
    def __init__(self, client, devicename, non_numeric_name, *numeric_names):
        ...
        self.num_macs, self.num_connects, self.num_unknowns = \
            self._get_streams(*numeric_names)

    def update_all(self, scanner):
        # Perform the scan
        scanner.scan()

        # If the connections change, update the stream.
        if (collections.Counter(scanner.previous_mac_vendors) !=
                collections.Counter(scanner.mac_vendors)):

            logger.info('Updating stream')
            posttime = datetime.now()

            values = {
                self.mac_addresses.name: [{
                    'timestamp': posttime,
                    'value': scanner.get_mac_vendors_json()
                }],
                self.num_macs.name: [{
                    'timestamp': posttime,
                    'value': scanner.known
                }],
                self.num_unknowns.name: [{
                    'timestamp': posttime,
                    'value': scanner.unknown
                }],
                self.num_connects.name: [{
                    'timestamp': posttime,
                    'value': scanner.total
                }]
            }
    ...
    def _get_device(self, devicename):
        try:
            device = [d for d in self.client.devices(q=devicename)
                      if d.name == devicename][0]
        except IndexError:
